...
sed -i 's/CONFIG_CC_WERROR=y/# CONFIG_CC_WERROR is not set/' arch/arm64/configs/${DEFCONFIG}
...
if [[ "$COMPILER" == "gcc" ]]; then
     ...
     MAKE+=( CROSS_COMPILE=aarch64-linux-gnu- CROSS_COMPILE_ARM32=arm-linux-gnueabi-)
     fi
fi
